...
fun RecipeExecutor.mvvmActivityRecipe(
    ...
    if (viewModelEnable||needPaging3Enable) {
        save(mvvmViewModel(basePackageName,packageName, activityClass,beanName,needPaging3Enable), srcOut.resolve("${activityClass}ViewModel.${ktOrJavaExt}"))
        if (repositoryEnable) {
            //repository
            save(mvvmRepository(packageName, activityClass,beanName,needPaging3Enable,pagingSourceName), srcOut.resolve("${activityClass}Repository.${ktOrJavaExt}"))
        }
    }
